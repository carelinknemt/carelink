import { Ambulance, CheckCircle2, ChevronRight, ArrowLeft, PhoneCall, Calculator, ShieldCheck, DollarSign } from 'lucide-react';
import React, { useState } from 'react';
import nemtBannerImg from '../assets/images/non-emergency-medical-transportation.png';
import { TRANSPORT_SERVICES, FAQS, COMPANY_INFO } from '../data/carelinkData';
...
export const ServicesPage: React.FC<ServicesPageProps> = ({
  ...
  const [calcVehicle, setCalcVehicle] = useState<'Wheelchair' | 'TransitShuttle' | 'Ambulatory'>('Wheelchair');
  const [calcMiles, setCalcMiles] = useState<number>(15);
  const [calcIsRoundTrip, setCalcIsRoundTrip] = useState<boolean>(true);
  const [calcCounty, setCalcCounty] = useState<string>('Humboldt');

  // Estimate calculation formula
  const baseRates = { Wheelchair: 55, TransitShuttle: 85, Ambulatory: 35 };
  const perMileRates = { Wheelchair: 3.5, TransitShuttle: 4.5, Ambulatory: 2.5 };
  const baseFare = baseRates[calcVehicle];
  const mileageFare = calcMiles * perMileRates[calcVehicle] * (calcIsRoundTrip ? 2 : 1);
  const totalEstimatedFare = Math.round(baseFare + mileageFare);
