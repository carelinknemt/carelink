import { MapPin, ChevronRight, Clock, PhoneCall, Mail, ShieldCheck } from 'lucide-react';
import React, { useState } from 'react';
import { COMPANY_INFO } from '../data/carelinkData';

interface ContactAndHoursProps {
  onOpenBooking: () => void;
  onBookDaySlot?: (day: string) => void;
}
            ...
            >
              ...
              <div className="absolute top-4 left-4 z-10 max-w-[280px] sm:max-w-xs rounded-2xl bg-white/95 p-4 shadow-xl backdrop-blur-md border border-slate-200">
                ...
                <div className="text-[10px] text-cyan-700 font-bold mt-1">
                  Primary Region: Humboldt, Del Norte, Trinity & Shasta
                </div>
              </div>
            </div>
            ...
            <div className="relative lg:absolute lg:top-6 lg:right-6 lg:bottom-6 lg:w-[400px] z-20 flex flex-col bg-[#004B87] lg:bg-[#004B87]/95 lg:backdrop-blur-md p-6 text-white shadow-2xl border-t lg:border border-cyan-400/30 overflow-y-auto">
              <div className="mb-3 pb-3 border-b border-cyan-400/30">
                ...
                <p className="text-[11px] text-cyan-100 mt-1">
                  Dependable hospital discharge & emergency ride dispatching.
                </p>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-cyan-400/30 text-xs space-y-1.5">
                <div className="flex items-center justify-between text-cyan-100">
                  ...
                  <a href={`tel:${COMPANY_INFO.dispatchPhone}`} className="font-bold text-white hover:underline">{COMPANY_INFO.dispatchPhone}</a>
                </div>
                <div className="flex items-center justify-between text-cyan-100">
                  <span>Email:</span>
                  <a href={`mailto:${COMPANY_INFO.email}`} className="font-bold text-cyan-200 hover:underline">{COMPANY_INFO.email}</a>
                </div>
              </div>
            </div>
