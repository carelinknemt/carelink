import { ShieldCheck, CheckCircle2, ChevronRight, ArrowLeft, Ambulance, Sparkles, UserCheck, Flame } from 'lucide-react';
...
export const FleetPage: React.FC<FleetPageProps> = ({
  ...
  const filteredFleet = VEHICLE_FLEET.filter(
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="space-y-2 pt-2 border-t border-slate-100">
                    ...
                    <ul className="text-xs text-slate-600 list-disc list-inside space-y-1">
                      {vehicle.accessibilitySpecs.map((spec, i) => (
                        <li key={i}>{spec}</li>
                      ))}
                    </ul>
                  </div>
        ...
        <div className="rounded-3xl bg-slate-900 text-white p-8 border border-slate-800 shadow-xl space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-xs text-slate-300">
            <div className="p-4 rounded-2xl bg-slate-800/80 border border-slate-700">
              ...
              <p className="text-[11px] leading-relaxed">Automatic 4-point wheelchair floor restraints rated to exceed Federal Motor Vehicle Safety Standards (FMVSS).</p>
            </div>

            <div className="p-4 rounded-2xl bg-slate-800/80 border border-slate-700">
              <div className="font-extrabold text-orange-400 mb-1">Sanitization Protocols</div>
              <p className="text-[11px] leading-relaxed">Hospital-grade interior UV light and disinfectant wipe-down after every single passenger transit.</p>
            </div>

            <div className="p-4 rounded-2xl bg-slate-800/80 border border-slate-700">
              <div className="font-extrabold text-orange-400 mb-1">GPS & Telemetry</div>
              <p className="text-[11px] leading-relaxed">Real-time Bambi NEMT GPS tracking and speed monitoring for passenger peace of mind.</p>
            </div>
          </div>
        </div>
