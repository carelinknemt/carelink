...
import womenCareImg from '../assets/images/A-women-sharing-joy-with-caregiver1.png';

interface SpecializedTeamProps {
  onSelectDoctor?: (member: any) => void;
  onSeeAllStaff?: () => void;
}

export const SpecializedTeam: React.FC<SpecializedTeamProps> = () => {
...
return;
  ...
  // Derived active values based on viewport (Mobile vs Desktop)
  const currentSectionMinHeight = '100dvh';
  ...
  const currentImageXOffset = 0;
  const currentImageYOffset = 0;
  ...
  return (
    <section 
      ...
      style={{ 
        minHeight: currentSectionMinHeight,
    ...
    >
      ...
      <div 
        ...
        style={{ 
          backgroundImage: `url(${isMobile ? 'https://i.postimg.cc/gjn121HQ/cover.png' : 'https://i.postimg.cc/jSrqBjZP/Img-Carelink.png'})`,
        }}
      />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-12 relative z-10 w-full">
        <div 
          ...
          style={{
            ...
            transform: `translateX(${currentCardXOffset}px) translateY(${currentCardYOffset}px)`
          }}
        >
          ...
          <div 
            className="absolute inset-x-0 bottom-0 rounded-b-3xl overflow-hidden pointer-events-none z-10"
          ...
          >
            <img 
              ...
              style={{
                ...
                transform: `translateX(calc(-50% + ${currentImageXOffset}px)) translateY(${currentImageYOffset}px) scale(${currentImageScale})`,
          ...
          <div className="relative z-20 mt-auto bg-white/90 backdrop-blur-xs px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-xl border border-orange-200/60 shadow-sm">
            <span className="text-[9px] sm:text-[10px] font-black text-[#E64A19] uppercase tracking-widest block">
              Transit Support
            </span>
          </div>
        </div>
